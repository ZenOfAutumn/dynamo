...
**注意：** 请确保 GitHub 分支/提交版本与 Dynamo 平台和 VLLM 容器相匹配。
...
[查看安装步骤](/docs/kubernetes/installation-guide.md#overview)
...
## 部署推理图（Inference Graph）

我们将向 Dynamo 平台部署一个 LLM 模型。这里以 `Qwen/Qwen3-0.6B` 模型 + VLLM + 解耦（disaggregated）部署为例。

在部署 yaml 文件中需要/可以做如下调整：

- **（必需）** 在 decoder 容器中添加修改 `LD_LIBRARY_PATH` 与 `PATH` 的参数，以便 GKE 找到正确的 GPU 驱动
- 将 VLLM 镜像更换为 NGC 上你期望使用的版本
- 在 metadata 中添加 namespace
- 调整 GPU/CPU 的 request 与 limit
- 修改要部署的模型

更多配置请参见 https://github.com/ai-dynamo/dynamo/tree/main/examples/deployments/GKE/vllm

### yaml 文件中的关键配置
请注意：在 GKE 中，必须按照 [Run GPUs in GKE](https://cloud.google.com/kubernetes-engine/docs/how-to/gpus) 正确设置 `LD_LIBRARY_PATH`。

部署 `yaml` 文件的 `args` 字段需要包含以下片段：
...
例如，参考 [`examples/deployments/GKE/vllm/disagg_gke.yaml`](./vllm/disagg_gke.yaml) 中的内容：
